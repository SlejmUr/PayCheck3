    ...
    public class GSController
    {
        ...
        public static void Make(MatchTickets.TicketReqJson ticketReq, ServerStruct serverStruct, string NameSpace)
        {
            ...
            var gs = new GameSession()
            {
                ...
                Configuration = CloudSaveDataHelper.GetStaticData<PartyPost.Configuration>($"PartyConfigurations/{ticketReq.matchPool}Configuration.json"),
        ...
        public static GameSession MakeP2P(CreateGaneSession createGaneSession, ServerStruct serverStruct, TokenHelper.Token token)
        {
            ...
            var gs = new GameSession()
            {
                ...
                Code = UserIdHelper.CreateCode(),
                Configuration = CloudSaveDataHelper.GetStaticData<PartyPost.Configuration>($"PartyConfigurations/{createGaneSession.ConfigurationName}Configuration.json"),
                CreatedAt = DateTime.UtcNow.ToString("yyyy-MM-ddTHH:mm:ssZ"),
                CreatedBy = "client-db" + UserIdHelper.CreateNewID(),
                Id = id,
                IsActive = true,
        ...
        public static GameSession Patch(PatchGameSessions patchGameSessions, string id, ServerStruct serverStruct, string NameSpace)
        {
            ...
            OnDSStatusChanged.Basic onDSStatusChanged = new()
            {
                ...
                Session = new()
                {
                    ...
                    Code = session.Code,
                    Configuration = CloudSaveDataHelper.GetStaticData<Dictionary<string, object>>("GameSessionConfigurations/Lobby_pveheist_DS.json"),
